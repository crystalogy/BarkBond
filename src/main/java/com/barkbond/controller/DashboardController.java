...
//    }
    ...
    public ModelAndView dashboard(@RequestParam(required = false) String term) {
        ModelAndView response = new ModelAndView("dashboard");
        List<Animal> animals = animalService.getAllAnimals();
        List<User> users = userService.getAllUsers();
        List<Animal> searchResults = animalDAO.findByCustomTerm(term);
        response.addObject("searchResults", searchResults);
...
//        Animal animal = animalDAO.findAnimalByAnimalID(id);
